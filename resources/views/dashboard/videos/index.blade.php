...
@section('page-header')
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="my-auto">
						<div class="d-flex">
							<h4 class="content-title mb-0 my-auto">{{ __('translation.videos') }}</h4>
						</div>
					</div>
				</div>
				<!-- breadcrumb -->
...
@section('content')
				...
				<div class="row row-sm">
					<!--div-->
					<div class="col-md-12">
                        @include('dashboard.videos.filter.filter')
						<div class="card card-primary">
							<div class="card-header py-3 d-flex">
                                <div class="d-flex justify-content-between">
                                    <a href="{{ route('dashboard.videos.create') }}" class="btn btn-primary btn-with-icon btn-block">
                                    ...
                                    <table id="datatable" class="table table-striped table-hover">
                                        ...
                                        <tbody>
                                            @foreach ($videos as $video)
                                                <tr>
                                                    <td><input type="checkbox" value="{{$video->id}}" class="box1"></td>
                                                    <td>
                                                        <img alt="{{ $video->title }}" class="rounded avatar-md mr-2" src="http://img.youtube.com/vi/{{ get_youtube_id($video->url) }}/hqdefault.jpg">
                                                    </td>
                                                    <td>{{ $video->title }}</td>
                                                    <td>{{ $video->video_section->title }}</td>
                                                    <td>
                                                    ...
                                                    <td>
                                                        <div class="btn-icon-list">
                                                            {{-- video edit --}}
                                                            <a href="{{ route('dashboard.videos.edit', $video->id) }}" class="btn btn-primary btn-icon"><i class="typcn typcn-pencil"></i></a>
                                                            {{-- video delete --}}
        ...
        <div class="modal fade" id="deleteModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <form action="{{ route('dashboard.videos.destroy_all') }}" method="POST">
                        @csrf
                        @method('DELETE')
