...
@section('page-header')
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="my-auto">
						<div class="d-flex">
							<h4 class="content-title mb-0 my-auto">{{ __('translation.photos') }}</h4>
						</div>
					</div>
				</div>
				<!-- breadcrumb -->
...
@section('content')
				...
				<div class="row row-sm">
					<!--div-->
					<div class="col-md-12">
                        @include('dashboard.photos.filter.filter')
						<div class="card card-primary">
							<div class="card-header py-3 d-flex">
                                <div class="d-flex justify-content-between">
                                    <a href="{{ route('dashboard.photos.create') }}" class="btn btn-primary btn-with-icon btn-block">
                                    ...
                                    <table id="datatable" class="table table-striped table-hover">
                                        ...
                                        <tbody>
                                            @foreach ($photos as $photo)
                                                <tr>
                                                    <td><input type="checkbox" value="{{$photo->id}}" class="box1"></td>
                                                    <td>
                                                        <img alt="{{ $photo->name }}" class="rounded avatar-md mr-2" src="{{ $photo->image_path  }}">
                                                    </td>
                                                    <td>{{ $photo->title }}</td>
                                                    <td>{{ $photo->photo_section->title }}</td>
                                                    <td>
                                                    ...
                                                    <td>
                                                        <div class="btn-icon-list">
                                                            {{-- photo edit --}}
                                                            <a href="{{ route('dashboard.photos.edit', $photo->id) }}" class="btn btn-primary btn-icon"><i class="typcn typcn-pencil"></i></a>
                                                            {{-- photo delete --}}
        ...
        <div class="modal fade" id="deleteModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <form action="{{ route('dashboard.photos.destroy_all') }}" method="POST">
                        @csrf
                        @method('DELETE')
