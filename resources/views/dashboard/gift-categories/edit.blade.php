...
@section('page-header')
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="my-auto">
						<div class="d-flex">
							<h4 class="content-title mb-0 my-auto">{{ __('translation.gift_categories') }}</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{ __('dashboard.update') }}</span>
						</div>
					</div>
				</div>
				<!-- breadcrumb -->
                ...
                <div class="row">
                    <div class="col-xl-12">
                        <div class="card card-primary">
                            ...
                            <div class="card-body">
                                {!! Form::open(['route' => ['dashboard.gift-categories.update', $category->id], 'method' => 'post', 'files' => true]) !!}
                                @csrf
                                @method('PATCH')
                                <div class="row">
                                    <div class="col-md-12">
                                ...
                                <div class="row mg-t-20">
                                    ...
                                    <div class="col-lg-3">
                                        <label class="rdiobox">
                                            {!! Form::radio('status', 'active', (old('status', $category->status)) == 'active' ? 'checked' : '') !!}
                                            <span>{{ __('translation.active') }}</span>
                                        </label>
                                    </div>
                                    <div class="col-lg-3 mg-t-20 mg-lg-t-0">
                                        <label class="rdiobox">
                                            {!! Form::radio('status', 'inactive', (old('status', $category->status)) == 'inactive' ? 'checked' : '') !!}
...
<script>
    $(document).ready(function() {
        $('#gift-cards').fileinput({
            theme: "fas",
            maxFileCount: {{ 8 - $category->cards->count() }},
            ...
            initialPreviewConfig: [
                    @if($category->cards->count() > 0)
                    @foreach($category->cards as $card)
                {caption: "{{ $card->file_name }}", size: {{ $card->file_size }}, width: "120px", url: "{{ route('dashboard.gift-categories.card.destroy', [$card->id, '_token' => csrf_token()]) }}", key: "{{ $card->id }}"},
                @endforeach
                @endif
            ],
        });
