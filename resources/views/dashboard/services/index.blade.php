...
@section('page-header')
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="my-auto">
						<div class="d-flex">
							<h4 class="content-title mb-0 my-auto">{{ __('translation.services') }}</h4>
						</div>
					</div>
				</div>
				<!-- breadcrumb -->
...
@section('content')
				...
				<div class="row row-sm">
					<!--div-->
					<div class="col-md-12">
                        @include('dashboard.services.filter.filter')
						<div class="card card-primary">
							<div class="card-header py-3 d-flex">
                                <div class="d-flex justify-content-between">
                                    <a href="{{ route('dashboard.services.create') }}" class="btn btn-primary btn-with-icon btn-block">
                                    ...
                                    <table id="datatable" class="table table-striped table-hover">
                                        ...
                                        <tbody>
                                            @foreach ($services as $service)
                                                <tr>
                                                    <td><input type="checkbox" value="{{$service->id}}" class="box1"></td>
                                                    <td>
                                                        <img alt="{{ $service->name }}" class="rounded avatar-md mr-2" src="{{ $service->image_path  }}">
                                                    </td>
                                                    <td>{{ $service->title }}</td>
                                                    <td>
                                                        @if ($service->service_section)
                                                            <a href="{{ route('dashboard.services.show', $service->service_section->id) }}">{{ $service->service_section->title }}</a>
                                                        @else
                                                            -
                                                        @endif
                                                    </td>
                                                    ...
                                                    <td>
                                                        <div class="btn-icon-list">
                                                            {{-- service edit --}}
                                                            <a href="{{ route('dashboard.services.edit', $service->id) }}" class="btn btn-primary btn-icon"><i class="typcn typcn-pencil"></i></a>
                                                            {{-- service delete --}}
        ...
        <div class="modal fade" id="deleteModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <form action="{{ route('dashboard.services.destroy_all') }}" method="POST">
                        @csrf
                        @method('DELETE')
