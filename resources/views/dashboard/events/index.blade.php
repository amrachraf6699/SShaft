...
@section('page-header')
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="my-auto">
						<div class="d-flex">
							<h4 class="content-title mb-0 my-auto">{{ __('translation.events') }}</h4>
						</div>
					</div>
				</div>
				<!-- breadcrumb -->
...
@section('content')
				...
				<div class="row row-sm">
					<!--div-->
					<div class="col-md-12">
                        @include('dashboard.events.filter.filter')
						<div class="card card-primary">
							<div class="card-header py-3 d-flex">
                                <div class="d-flex justify-content-between">
                                    <a href="{{ route('dashboard.events.create') }}" class="btn btn-primary btn-with-icon btn-block">
                                    ...
                                    <table id="datatable" class="table table-striped table-hover">
                                        <thead class="">
                                            <tr>
                                                ...
                                                <th class="py-2">{{ __('translation.location') }}</th>
                                                <th class="py-2">{{ __('translation.date_of_event') }}</th>
                                                <th class="py-2">{{ __('translation.time_of_event') }}</th>
                                        ...
                                        <tbody>
                                            @foreach ($events as $event)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <img alt="{{ $event->name }}" class="rounded avatar-md mr-2" src="{{ $event->image_path  }}">
                                                    </td>
                                                    <td>{{ $event->title }}</td>
                                                    ...
                                                    <td>
                                                        <div class="btn-icon-list">
                                                            {{-- event edit --}}
                                                            <a href="{{ route('dashboard.events.edit', $event->id) }}" class="btn btn-primary btn-icon"><i class="typcn typcn-pencil"></i></a>
                                                            {{-- event delete --}}
        ...
        <div class="modal fade" id="deleteModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <form action="{{ route('dashboard.events.destroy_all') }}" method="POST">
                        @csrf
                        @method('DELETE')
