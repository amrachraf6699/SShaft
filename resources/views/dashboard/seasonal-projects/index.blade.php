...
@section('page-header')
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="my-auto">
						<div class="d-flex">
							<h4 class="content-title mb-0 my-auto">{{ __('translation.seasonal_projects') }}</h4>
						</div>
					</div>
				</div>
				<!-- breadcrumb -->
...
@section('content')
				...
				<div class="row row-sm">
					<!--div-->
					<div class="col-md-12">
                        @include('dashboard.seasonal-projects.filter.filter')
						<div class="card card-primary">
							<div class="card-header py-3 d-flex">
                                <div class="d-flex justify-content-between">
                                    <a href="{{ route('dashboard.seasonal-projects.create') }}" class="btn btn-primary btn-with-icon btn-block">
                                    ...
                                    <table id="datatable" class="table table-striped table-hover">
                                        ...
                                        <tbody>
                                            @foreach ($projects as $project)
                                                <tr>
                                                    <td><input type="checkbox" value="{{$project->id}}" class="box1"></td>
                                                    <td>
                                                        <img alt="{{ $project->name }}" class="rounded avatar-md mr-2" src="{{ $project->image_path  }}">
                                                    </td>
                                                    <td>{{ $project->title }}</td>
                                                    <td>
                                                    ...
                                                    <td>
                                                        <div class="btn-icon-list">
                                                            {{-- project edit --}}
                                                            <a href="{{ route('dashboard.seasonal-projects.edit', $project->id) }}" class="btn btn-primary btn-icon"><i class="typcn typcn-pencil"></i></a>
                                                            {{-- project delete --}}
                                                            <form action="{{ route('dashboard.seasonal-projects.destroy', $project->id) }}" method="post" style="display: inline-block">
                                                                @csrf
                                                                @method('DELETE')
        ...
        <div class="modal fade" id="deleteModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <form action="{{ route('dashboard.seasonal-projects.destroy_all') }}" method="POST">
                        @csrf
                        @method('DELETE')
