...
@section('page-header')
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="my-auto">
						<div class="d-flex">
							<h4 class="content-title mb-0 my-auto">{{ __('translation.verification_donations') }}</h4>
						</div>
					</div>
				</div>
				<!-- breadcrumb -->
...
@section('content')
				...
				<div class="row row-sm">
					<!--div-->
					<div class="col-md-12">
                        @include('dashboard.verification-donations.filter.filter')
						<div class="card card-primary">
							<div class="card-header py-3 d-flex">
                                <div class="d-flex justify-content-between">
                                    <a href="{{ route('dashboard.verification-donations.export') }}" class="btn btn-purple btn-with-icon mr-2">
                                    ...
                                    <table id="datatable" class="table table-striped table-hover">
                                        <thead class="">
                                            <tr>
                                                ...
                                                <th class="py-2">{{ __('translation.bank_name') }}</th>
                                                <th class="py-2">{{ __('translation.transfer_receipt') }}</th>
                                        ...
                                        <tbody>
                                            @foreach ($donations as $donation)
                                                <tr>
                                                    ...
                                                    <td>{{ $donation->donor->membership_no ?? __('translation.null') }} <br /> <span dir="ltr">{{ json_decode($donation->response)->cardNumber ?? __('translation.null') }}</span></td>
                                                    ...
                                                    <td><a href="{{ $donation->transfer_receipt }}" target="_blank"><i class="text-purple las la-link la-2x"></i></a></td>
                                                    <td>
                                                    ...
                                                    <td>
                                                        <div class="btn-icon-list">
                                                            ...
                                                            @if ($donation->payment_ways == 'bank_transfer')
                                                                <a href="{{ route('dashboard.verification-donations.edit', $donation->id) }}" class="btn btn-primary btn-icon"><i class="typcn typcn-pencil"></i></a>
                                                            @endif
                                                            {{-- donation delete --}}
                                                            <form action="{{ route('dashboard.verification-donations.destroy', $donation->id) }}" method="post" style="display: inline-block">
                                                                @csrf
                                                                @method('DELETE')
        ...
        <div class="modal fade" id="deleteModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <form action="{{ route('dashboard.verification-donations.destroy_all') }}" method="POST">
                        @csrf
                        @method('DELETE')
