...
@section('page-header')
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="my-auto">
						<div class="d-flex">
							<h4 class="content-title mb-0 my-auto">{{ __('translation.accounts') }}</h4>
						</div>
					</div>
				</div>
				<!-- breadcrumb -->
...
@section('content')
				...
				<div class="row row-sm">
					<!--div-->
					<div class="col-md-12">
                        @include('dashboard.accounts.filter.filter')
						<div class="card card-primary">
							<div class="card-header py-3 d-flex">
                                <div class="d-flex justify-content-between">
                                    <a href="{{ route('dashboard.accounts.create') }}" class="btn btn-primary btn-with-icon btn-block">
                                    ...
                                    <table id="datatable" class="table table-striped table-hover">
                                        <thead class="">
                                            <tr>
                                                ...
                                                <th class="py-2">{{ __('translation.account_number') }}</th>
                                                <th class="py-2">{{ __('translation.IBAN') }}</th>
                                                <th class="py-2">{{ __('translation.bank_link') }}</th>
                                        ...
                                        <tbody>
                                            @foreach ($accounts as $branch)
                                                <tr>
                                                    <td><input type="checkbox" value="{{$branch->id}}" class="box1"></td>
                                                    <td>{{ $branch->bank_name }}</td>
                                                    <td>{{ $branch->account_number }}</td>
                                                    <td>{{ $branch->IBAN }}</td>
                                                    <td>
                                                        @if ($branch->bank_link)
                                                            <a href="{{ $branch->bank_link }}" target="_blank"><i class="text-info las la-link la-2x"></i></a>
                                                        @else
                                                            -
                                                        @endif
                                                    </td>
                                                    ...
                                                    <td>
                                                        <div class="btn-icon-list">
                                                            {{-- branch edit --}}
                                                            <a href="{{ route('dashboard.accounts.edit', $branch->id) }}" class="btn btn-primary btn-icon"><i class="typcn typcn-pencil"></i></a>
                                                            {{-- branch delete --}}
                                                            <form action="{{ route('dashboard.accounts.destroy', $branch->id) }}" method="post" style="display: inline-block">
                                                                @csrf
                                                                @method('DELETE')
        ...
        <div class="modal fade" id="deleteModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <form action="{{ route('dashboard.accounts.destroy_all') }}" method="POST">
                        @csrf
                        @method('DELETE')
