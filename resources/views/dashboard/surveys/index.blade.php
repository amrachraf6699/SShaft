...
@section('page-header')
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="my-auto">
						<div class="d-flex">
							<h4 class="content-title mb-0 my-auto">{{ __('translation.surveys') }}</h4>
						</div>
					</div>
				</div>
				<!-- breadcrumb -->
...
@section('content')
				...
				<div class="row row-sm">
					<!--div-->
					<div class="col-md-12">
                        @include('dashboard.surveys.filter.filter')
						<div class="card card-primary">
							<div class="card-header py-3 d-flex">
                                <div class="d-flex justify-content-between">
                                    <a href="{{ route('dashboard.surveys.create') }}" class="btn btn-primary btn-with-icon btn-block">
                                    ...
                                    <table id="datatable" class="table table-striped table-hover">
                                        <thead class="">
                                            <tr>
                                                ...
                                                <th class="py-2">{{ __('translation.total_votes') }}</th>
                                                <th class="py-2">{{ __('translation.very_satisfied') }}</th>
                                                <th class="py-2">{{ __('translation.somewhat_satisfied') }}</th>
                                                <th class="py-2">{{ __('translation.neutral') }}</th>
                                                <th class="py-2">{{ __('translation.not_satisfied') }}</th>
                                                <th class="py-2">{{ __('translation.angry') }}</th>
                                        ...
                                        <tbody>
                                            @foreach ($surveys as $survey)
                                                <tr>
                                                    <td><input type="checkbox" value="{{$survey->id}}" class="box1"></td>
                                                    <td>{{ $survey->title }}</td>
                                                    <td>
                                                    ...
                                                    <td>{{ round($survey->percent_very_satisfied, 2) }} %</td>
                                                    <td>{{ round($survey->percent_percent_somewhat_satisfied, 2) }} %</td>
                                                    <td>{{ round($survey->percent_neutral, 2) }} %</td>
                                                    <td>{{ round($survey->percent_not_satisfied, 2) }} %</td>
                                                    <td>{{ round($survey->percent_angry, 2) }} %</td>
                                                    <td>{{ $survey->created_at->format('M d, Y') }}</td>
                                                    <td>
                                                        <div class="btn-icon-list">
                                                            {{-- surveys edit --}}
                                                            <a href="{{ route('dashboard.surveys.edit', $survey->id) }}" class="btn btn-primary btn-icon"><i class="typcn typcn-pencil"></i></a>
                                                            {{-- surveys delete --}}
                                                            <form action="{{ route('dashboard.surveys.destroy', $survey->id) }}" method="post" style="display: inline-block">
                                                                @csrf
                                                                @method('DELETE')
        ...
        <div class="modal fade" id="deleteModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <form action="{{ route('dashboard.surveys.destroy_all') }}" method="POST">
                        @csrf
                        @method('DELETE')
