...
@section('page-header')
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="my-auto">
						<div class="d-flex">
							<h4 class="content-title mb-0 my-auto">{{ __('translation.blogs') }}</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{ __('translation.sections') }}</span>
						</div>
					</div>
				</div>
				<!-- breadcrumb -->
...
@section('content')
				...
				<div class="row row-sm">
					<!--div-->
					<div class="col-md-12">
                        @include('dashboard.blogs.sections.filter.filter')
						<div class="card card-primary">
							<div class="card-header py-3 d-flex">
                                <div class="d-flex justify-content-between">
                                    <a href="{{ route('dashboard.blog-sections.create') }}" class="btn btn-primary btn-with-icon btn-block">
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <div class="btn-icon-list">
                                                            {{-- section edit --}}
                                                            <a href="{{ route('dashboard.blog-sections.edit', $section->id) }}" class="btn btn-primary btn-icon"><i class="typcn typcn-pencil"></i></a>
                                                            {{-- section delete --}}
                                                            <form action="{{ route('dashboard.blog-sections.destroy', $section->id) }}" method="post" style="display: inline-block">
                                                                @csrf
                                                                @method('DELETE')
        ...
        <div class="modal fade" id="deleteModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <form action="{{ route('dashboard.blog-sections.destroy_all') }}" method="POST">
                        @csrf
                        @method('DELETE')
