...
@section('page-header')
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="my-auto">
						<div class="d-flex">
							<h4 class="content-title mb-0 my-auto">{{ __('translation.blogs') }}</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{ __('dashboard.update') }}</span>
						</div>
					</div>
				</div>
				<!-- breadcrumb -->
                        ...
                        <div class="card card-primary">
                            <div class="card-header py-3 d-flex">
                                <div class="d-flex justify-content-between">
                                    <a href="{{  route('dashboard.blogs.index') }}" class="btn btn-primary btn-with-icon btn-block">
                                        {{ __('translation.blogs') }}
                                    </a>
                                </div>
                            </div>
                            <div class="card-body">
                                {!! Form::open(['route' => ['dashboard.blogs.update', $blog->id], 'method' => 'post', 'files' => true]) !!}
                                @csrf
                                @method('PATCH')
                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            {!! Form::label('blog_section_id', trans('translation.section')) !!}
                                            {!! Form::select('blog_section_id', ['' => trans('dashboard.category_select')] + $sections->toArray(), old('blog_section_id', $blog->blog_section_id), ['class' => 'form-control select2']) !!}
                                            @error('blog_section_id')<span class="text-danger">{{ $message }}</span>@enderror
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            ...
                                            {!! Form::textarea('content', old('content', $blog->content), ['class' => 'form-control summernote', 'rows' => 4]) !!}
                                ...
                                <div class="row mg-t-20">
                                    ...
                                    <div class="col-lg-3">
                                        <label class="rdiobox">
                                            {!! Form::radio('status', 'active', (old('status', $blog->status)) == 'active' ? 'checked' : '') !!}
                                            <span>{{ __('translation.active') }}</span>
                                        </label>
                                    </div>
                                    <div class="col-lg-3 mg-t-20 mg-lg-t-0">
                                        <label class="rdiobox">
                                            {!! Form::radio('status', 'inactive', (old('status', $blog->status)) == 'inactive' ? 'checked' : '') !!}
                                ...
                                <div class="row pt-4">
                                    <div class="col-12">
                                        ...
                                        <br>
                                        <input type="file" name="img" class="dropify" data-default-file="{{ $blog->image_path }}" accept=".jpg, .png, image/jpeg, image/png" data-height="70" />
