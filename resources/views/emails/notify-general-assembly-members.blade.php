                                ...
                                <table class="row" align="center" cellpadding="0" cellspacing="0" role="presentation">
                                    <tr>
                                        <th class="column" width="640" style="text-align: right;">
                                            <div class="serif" style="color: #1F2225; font-size: 28px; font-weight: 700; line-height: 50px; margin-bottom: 30px; margin-top:20px">مرحباً {{ $member['name'] }}</div>
                                            <div class="sans-serif" style="font-size: 18px; font-weight: 400; line-height: 28px; margin-bottom: 40px;direction: rtl;">
                                                @if ($member['status'] == 'awaiting_payment')
                                                    <p>
                                                        تهانينا، لقد تمت المراجعة والتأكد من صحة بيانتكم المقدمة إلينا للإنضام إلي أعضاء الجمعية العمومية. <br>
                                                        وبناء علي ماسبق تدعوكم {{ $setting->name }} إلي تفحص الرمز التالي أو الضغط علي الرابط أدناه ليتم توجيهكم إلي صفحة إتمام عملية الدفع الخاصة بإشتراك أعضاء جمعية عمومية ودفع مبلغ وقدرة <strong>"{{ $member['total_amount'] }} ريال سعودي"</strong> حتي نتمكن من تفعيل عضويتكم وإدراجها ضمن أعضاء الجمعية العمومية لدينا. <br>
                                                        نشكر لكم ثقتكم ب{{ $setting->name }}.
                                                    </p>
                                                    <img src="https://api.qrserver.com/v1/create-qr-code/?size=100x100&data={{ route('frontend.pay-general-assembly-members.choose-payment-method.view', [$member['uuid'], $member['invoice_no']]) }}" alt="Full Width Image" width="20%" style="display:block; margin: 0 auto; margin-bottom:20px">
                                                    <p>
                                                        رابط إتمام عملية الدفع: <br>
                                                        <a href="{{ route('frontend.pay-general-assembly-members.choose-payment-method.view', [$member['uuid'], $member['invoice_no']]) }}">
                                                            {{ route('frontend.pay-general-assembly-members.choose-payment-method.view', [$member['uuid'], $member['invoice_no']]) }}
                                                        </a>
                                                    </p>
                                                @elseif ($member['status'] == 'active')
                                                    ...
                                                    <p>
                                                        كود العضوية: {{ $member['membership_no'] }} <br>
                                                        رابط شهادة عضو جمعية عمومية: <a href="{{ route('frontend.certificate-of-general-assembly-member.show', $member['uuid']) }}">
                                                                                    {{ route('frontend.certificate-of-general-assembly-member.show', $member['uuid']) }}
                                                                                </a> <br>
                                                    </p>
                                                    <img src="https://api.qrserver.com/v1/create-qr-code/?size=100x100&data={{ route('general-assembly-member-invoice.show', [$member['invoice_no'], $member['uuid']]) }}" alt="Full Width Image" width="20%" style="display:block; margin: 0 auto; margin-bottom:20px">
                                                    <p>
                                                        رابط الفاتورة: <br>
                                                        <a href="{{ route('general-assembly-member-invoice.show', [$member['invoice_no'], $member['uuid']]) }}">
                                                            {{ route('general-assembly-member-invoice.show', [$member['invoice_no'], $member['uuid']]) }}
                                                        </a>
                                                    </p>
                                                @elseif ($member['status'] == 'inactive')
                                                    <p>
                                                        تم إيقاف تفعيل عضوية جمعية عمومية الخاصة بكم، لمعرفة المزيد حول هذا الأمر يرجي التوصل مع الإدارة. <br>
                                                        نشكر لكم ثقتكم ب{{ $setting->name }}.
                                                    </p>
                                                @elseif ($member['status'] == 'pending')
                                                    <p>
                                                        تم إنشاء عضوية جمعية عمومية بنجاح وهي الآن {{ __('dashboard.pending') }} وفي إنتظار الموافقة من إدارة {{ $setting->name }}. <br>
                                                        في حال المراجعة والتأكد من صحة وسلامة بيانتكم سيتم إرسال رسالة مدرج بها رابط الدفع الخاص بالإشتراك السنوي لأعضاء الجمعية العمومية. <br>
                                                        نشكر لكم ثقتكم ب{{ $setting->name }}.
                                                    </p>
                                                @elseif ($member['status'] == 'rejected')
                                                    <p>
                                                        تم رفض عضويتكم من قبل الإدارة ولمزيد من التفاصيل يرجي التواصل مع إدارة {{ $setting->name }}.  <br>
                                                        نشكر لكم ثقتكم ب{{ $setting->name }}.
                                                    </p>
                                                @endif
                                            </div>
